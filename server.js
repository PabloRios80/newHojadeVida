...
app.get("/:origenCorto", (req, res, next) => {
  const clave = req.params.origenCorto.toLowerCase();
  if (ORIGENES_CORTOS[clave]) {
    return res.redirect(
      `/?origen=${encodeURIComponent(ORIGENES_CORTOS[clave])}`,
    );
  }
  next(); // Si no matchea ningún origen conocido, seguir con el resto (servirá el 404 normal o el estático si corresponde)
});
...
app.post("/saveData", async (req, res) => {
  ...
  try {
    ...
    const { error } = await supabase.from("afiliados").upsert(
      {
        dni: d.DNI,
        nombre: d.Nombre,
        apellido: d.Apellido,
    ...
    axios
      .get(`http://localhost:${PORT}/getPreventivePlan/${d.DNI}`)
      .then((r) =>
        console.log(
          `✅ Plan generado para ${d.DNI}: ${r.data.autorizadas || 0} prácticas`,
        ),
      )
...
app.get("/getPreventivePlan/:dni", async (req, res) => {
  ...
  try {
    ...
    if (practicasAutorizar.length === 0) {
      return res.json({
        success: true,
        message: "El afiliado está al día con todas sus prácticas.",
        autorizadas: 0,
      });
    }
  ...
  for (const regla of reglas) {
    ...
    if (regla.historial_condicion_campo && regla.historial_condicion_valor) {
      ...
      if (campoHistorial && ultimoDP) {
        const valorHistorial = (ultimoDP[campoHistorial] || "").toLowerCase();
        const valoresRequeridos = regla.historial_condicion_valor
          .toLowerCase()
          .split(",")
          .map((v) => v.trim());
        if (!valoresRequeridos.some((v) => valorHistorial.includes(v)))
          continue;
      }
    }
...
function buscarUltimaRealizacion(practica, practicasHistoricas, historial) {
  ...
  if (tipoPractica) {
    const encontrada = practicasHistoricas.find(
      (p) => p.tipo_practica === tipoPractica && p.fecha,
    );
...
app.get("/getPendingPractices/:dni", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error al buscar prácticas." });
  }
});
...
app.post("/savePracticeResult", async (req, res) => {
  ...
  try {
    ...
    if (existente) {
      ...
      res.json({
        success: false,
        message: "Práctica no encontrada o ya realizada.",
      });
    }
  } catch (error) {
  ...
  try {
    ...
    for (const p of practicas) {
      ...
      if (!practicasDedup[key]) {
        ...
        if (vNuevo.includes("DETECTABLE") && !vNuevo.includes("NO DETECTABLE"))
          practicasDedup[key] = p;
...
app.post("/saveDataMenores", async (req, res) => {
  const d = req.body;
  try {
    const { error } = await supabase.from("afiliados_menores").insert({
      dni: d.dni,
      nombre: d.nombre,
      apellido: d.apellido,
      fecha_nacimiento: d.fecha_nacimiento,
      ...
      telefono: d.telefono,
      email: d.email,
      fam_hipertension: d.fam_hipertension,
      fam_diabetes: d.fam_diabetes,
      fam_obesidad: d.fam_obesidad,
      fam_cardio: d.fam_cardio,
      fam_mental: d.fam_mental,
      fam_adicciones: d.fam_adicciones,
      fam_cancer: d.fam_cancer,
      act_fisica: d.act_fisica,
      ...
      vision: d.vision,
      audicion: d.audicion,
      escoliosis: d.escoliosis,
      alim_trastorno: d.alim_trastorno,
      menarca: d.menarca,
      ciclos: d.ciclos,
      vinculos: d.vinculos,
      tristeza: d.tristeza,
      atencion_mental: d.atencion_mental,
      violencia: d.violencia,
      tabaco: d.tabaco,
      alcohol: d.alcohol,
      sustancias: d.sustancias,
      educ_sexual: d.educ_sexual,
      origen_carga: d.origen_carga || null,
      dudas: d.dudas,
    ...
    axios
      ...
      .catch((e) =>
        console.warn("Backup Google Sheets menores falló:", e.message),
      );
...
app.post("/saveDataMenoresA", async (req, res) => {
  ...
  try {
    const { error } = await supabase.from("afiliados_menores").insert({
      ...
      pantallas: d.pantallas,
      lenguaje: d.lenguaje,
      motricidad: d.motricidad,
      interaccion: d.interaccion,
      esfinteres: d.esfinteres,
      escuela: d.escuela,
      vacunas: d.vacunas,
    ...
    axios
      ...
      .catch((e) =>
        console.warn("Backup Google Sheets menores A falló:", e.message),
      );
